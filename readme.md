...
可以看到，上面步骤中，重置密码非常重要的就是这个链接的加密，如果使用上述token的话，实际上是可以进行解密的，解密后通过 userId 即可生成一个新的链接，然后重置这个用户的密码，这样也就造成了一个任意重置密码的漏洞。所以重置密码，这个链接的生成必须是不可逆的，甚至要加入时间的元素。 过一定时间后，重置密码的链接就会失效，这里结合pycryptodemo 来实现
